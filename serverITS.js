    ...
    try {
      ...
      try {
        ...
        const archiveData = {
          ...
          metadata: {
            archived_from_url: "/html/interfaceFormulaireEmployer.html",
        ...
        await pool.query(insertArchiveQuery, [
...
// ===============================
// ROUTES SPÉCIALES POUR LES LIVRAISONS EN COURS (ONGLET MIS EN LIVRAISON)
// ===============================

// Récupérer les dossiers mis en livraison (pas encore livrés)
app.get("/api/deliveries/shipping-status", async (req, res) => {
  try {
    console.log("[API] Récupération des dossiers mis en livraison...");

    const query = `
      SELECT 
        id, employee_name, delivery_date, delivery_time, client_name, client_phone,
        container_type_and_content, lieu, status, created_at, updated_at,
        container_number, container_foot_type, declaration_number, number_of_containers,
        bl_number, dossier_number, shipping_company, transporter,
        weight, ship_name, circuit, delivery_status_acconier,
        observation_acconier, container_numbers_list, container_foot_types_map
      FROM livraison_conteneur 
      WHERE delivery_status_acconier = 'mise_en_livraison_acconier'
        AND (status != 'delivered' AND status != 'livré' AND delivery_status_acconier != 'delivered')
      ORDER BY created_at DESC
    `;
...
// Compter les dossiers mis en livraison
app.get("/api/deliveries/shipping-count", async (req, res) => {
  try {
    const query = `
    ...
    console.log(`[API] Comptage des dossiers mis en livraison: ${count}`);
