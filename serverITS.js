...
// ===============================
// ROUTE : Création d'un ordre de livraison (POST AJAX + WebSocket temps réel)
// ===============================
app.post("/ordre-livraison", async (req, res) => {
  try {
    // Récupération des champs attendus (adapter selon le formulaire)
    const {
      employee_name,
      client_name,
      client_phone,
    ...
    // Validation minimale (adapter selon besoins)
    if (
      !employee_name ||
      !client_name ||
      !container_type_and_content ||
    ...
    // Normalisation des champs date/heure
    const validated_delivery_date =
      typeof formatDateForDB === "function"
        ? formatDateForDB(delivery_date)
        : delivery_date;
    const validated_delivery_time =
      typeof formatTimeForDB === "function"
        ? formatTimeForDB(delivery_time)
        : delivery_time;

    // Normalisation du champ container_number (CSV ou tableau)
    ...
    const usedStatus = status || "pending_acconier";
    ...
    const query = `
      INSERT INTO livraison_conteneur (
        ...
        delivery_notes, delivery_status_acconier
      ) VALUES (
    ...
    res.status(201).json({
      success: true,
      message: "Ordre de livraison enregistré avec succès !",
      livraison: newDelivery,
    });
  } catch (err) {
    console.error(
      "Erreur lors de l'enregistrement de l'ordre de livraison:",
      err
    );
    res.status(500).json({
      success: false,
      message:
        "Erreur serveur lors de l'enregistrement de l'ordre de livraison.",
    });
  }
});
