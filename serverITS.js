...
// Route pour mettre à jour les dates lors de la mise en livraison
app.patch("/deliveries/:id/mise-en-livraison", async (req, res) => {
  const { id } = req.params;
  const { date_do, date_badt, date_paiement_acconage, date_echange_bl } =
    req.body;

  try {
    // Construction de la requête avec les champs à mettre à jour
    let updateFields = [];
    let values = [];
    let paramCount = 1;

    if (date_do) {
      updateFields.push(`date_do = $${paramCount}`);
      values.push(date_do);
      paramCount++;
    }

    if (date_badt) {
      updateFields.push(`date_badt = $${paramCount}`);
      values.push(date_badt);
      paramCount++;
    }

    if (date_paiement_acconage) {
      updateFields.push(`date_paiement_acconage = $${paramCount}`);
      values.push(date_paiement_acconage);
      paramCount++;
    }

    if (date_echange_bl) {
      updateFields.push(`date_echange_bl = $${paramCount}`);
      values.push(date_echange_bl);
      paramCount++;
    }

    // Ajout du statut de livraison
    updateFields.push(`delivery_status_acconier = $${paramCount}`);
    values.push("mise_en_livraison_acconier");
    ...
    const updateQuery = `
      ...
      RETURNING id, date_do, date_badt, date_paiement_acconage, date_echange_bl, delivery_status_acconier
    `;
    ...
    // Broadcast WebSocket pour notification en temps réel
    wsClients.forEach((client) => {
      if (client.readyState === WebSocket.OPEN) {
        client.send(
          JSON.stringify({
            type: "mise_en_livraison_update",
    ...
    console.error(
      "Erreur lors de la mise à jour des dates de mise en livraison:",
      err
    );
    res.status(500).json({
      success: false,
      message: "Erreur serveur lors de la mise à jour des dates.",
    });
  }
});
