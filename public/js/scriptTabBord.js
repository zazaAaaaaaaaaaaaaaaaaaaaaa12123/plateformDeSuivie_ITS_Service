...
function chargerStatistiquesActeurs() {
  ...
  if (!section) return;
  // Ajout du sélecteur de date dans la même ligne que les cartes acteurs (en haut à droite)
  ...
  // On ne met pas de value par défaut, il sera géré dynamiquement
  let dateSelectorHTML = `<div id='date-filtre-statistiques-container' style="display:flex;justify-content:center;align-items:center;gap:10px;margin-bottom:10px;margin-top:8px;">
                <label for='date-filtre-statistiques' style='font-weight:600;color:#2563eb;font-size:1.01em;'>Filtrer par date :</label>
                <input type='date' id='date-filtre-statistiques' max='${todayStr}' style='padding:4px 8px;font-size:0.97em;border:1px solid #d1d5db;border-radius:8px;min-width:110px;height:32px;'>
              </div>`;
  ...
  function fetchStats(dateStr) {
    ...
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        // Gestion du cas "date future" (date sélectionnée > aujourd'hui)
        ...
        if (selectedDate > now) {
          section.innerHTML = `
                        ${dateSelectorHTML}
                        <h2 style='font-size:2.2em;font-weight:800;color:#2563eb;margin-bottom:18px;text-align:center;letter-spacing:1px;text-shadow:0 2px 16px #2563eb22;display:flex;align-items:center;justify-content:center;gap:16px;'><i class='fas fa-chart-pie' style='color:#6366f1;font-size:1.1em;'></i> Statistiques par acteur</h2>
                        <div style='display:flex;justify-content:center;align-items:center;height:180px;'>
                          <div style='background:#f7faff;border-radius:16px;padding:38px 32px 28px 32px;box-shadow:0 4px 18px #2563eb18,0 2px 8px #0001;max-width:480px;width:96vw;text-align:center;'>
                            <i class='fas fa-calendar-times' style='font-size:2.2em;color:#dc3545;margin-bottom:12px;'></i><br>
                            <span style='font-size:1.25em;font-weight:700;color:#dc3545;'>Vous avez sélectionné une date future.</span><br>
                            <span style='font-size:1.05em;color:#374151;'>Aucune statistique n'est disponible pour une date ultérieure à aujourd'hui.</span>
                          </div>
                        </div>
                      `;
          // Réactiver le sélecteur de date même en cas de date future
          const dateSelector = document.getElementById(
            "date-filtre-statistiques"
          );
          if (dateSelector) {
            dateSelector.setAttribute("max", todayStr);
            if (!dateSelector.value) dateSelector.value = todayStr;
            dateSelector.removeEventListener(
              "change",
              dateSelector._statsChangeHandler || (() => {})
            );
            dateSelector._statsChangeHandler = function () {
              let val = dateSelector.value;
        ...
        // Gestion du cas "aucune donnée" (backend renvoie success: false et message)
        if (
          !data.success &&
          data.message &&
          data.message.toLowerCase().includes("aucune donnée")
        ) {
          section.innerHTML = `
                        ${dateSelectorHTML}
                        <h2 style='font-size:2.2em;font-weight:800;color:#2563eb;margin-bottom:18px;text-align:center;letter-spacing:1px;text-shadow:0 2px 16px #2563eb22;display:flex;align-items:center;justify-content:center;gap:16px;'><i class='fas fa-chart-pie' style='color:#6366f1;font-size:1.1em;'></i> Statistiques par acteur</h2>
                        <div style='display:flex;justify-content:center;align-items:center;height:180px;'>
                          <div style='background:#f7faff;border-radius:16px;padding:38px 32px 28px 32px;box-shadow:0 4px 18px #2563eb18,0 2px 8px #0001;max-width:480px;width:96vw;text-align:center;'>
                            <i class='fas fa-info-circle' style='font-size:2.2em;color:#2563eb;margin-bottom:12px;'></i><br>
                            <span style='font-size:1.25em;font-weight:700;color:#2563eb;'>Aucune donnée pour la date choisie.</span><br>
                            <span style='font-size:1.05em;color:#374151;'>Essayez une autre date ou vérifiez les opérations enregistrées ce jour-là.</span>
                          </div>
                        </div>
                      `;
          // Réactiver le sélecteur de date même en cas d'absence de données
          const dateSelector = document.getElementById(
            "date-filtre-statistiques"
          );
          if (dateSelector) {
            dateSelector.setAttribute("max", todayStr);
            if (!dateSelector.value) dateSelector.value = dateStr || todayStr;
            dateSelector.removeEventListener(
              "change",
              dateSelector._statsChangeHandler || (() => {})
            );
          ...
          section.innerHTML = `<div style='color:#dc3545;text-align:center;'>Erreur lors du chargement des statistiques.</div>`;
          return;
        }
        const acteurs = [
          {
            ...
            color: "#2563eb",
            icon: "fa-user-tie",
            bg: "linear-gradient(135deg, #eaf1fb 60%, #dbeafe 100%)",
            shadow: "0 8px 32px #2563eb22, 0 2px 8px #0001",
          },
          {
            key: "responsableAcconier",
            label: "Responsable Acconier",
            color: "#059669",
            icon: "fa-user-shield",
            bg: "linear-gradient(135deg, #e7f9f3 60%, #bbf7d0 100%)",
            shadow: "0 8px 32px #05966922, 0 2px 8px #0001",
          },
          {
            key: "responsableLivraison",
            label: "Responsable Livraison",
            color: "#f59e0b",
            icon: "fa-user-cog",
            bg: "linear-gradient(135deg, #fff7e6 60%, #fde68a 100%)",
            shadow: "0 8px 32px #f59e0b22, 0 2px 8px #0001",
          },
        ];
        let html = dateSelectorHTML;
        html += `<h2 style='font-size:2.2em;font-weight:800;color:#2563eb;margin-bottom:18px;text-align:center;letter-spacing:1px;text-shadow:0 2px 16px #2563eb22;display:flex;align-items:center;justify-content:center;gap:16px;'><i class='fas fa-chart-pie' style='color:#6366f1;font-size:1.1em;'></i> Statistiques par acteur</h2>`;
        html += `<div style="display:flex;align-items:flex-start;gap:0 18px;margin-bottom:0;">
                      <div class='dashboard-cards-row' id='statistiquesActeursRow' style='gap:12px;margin-bottom:0;flex:1 1 0;min-height:140px;'>`;
        acteurs.forEach((acteur) => {
          const stats = data[acteur.key] || {};
          html += `<div class='dashboard-card stats-acteur-card' tabindex='0' data-acteur='${acteur.key}' style='border-top:5px solid ${acteur.color};background:${acteur.bg};box-shadow:${acteur.shadow};cursor:pointer;min-height:110px;max-height:150px;max-width:220px;padding:8px 6px 7px 6px;margin:0;position:relative;overflow:hidden;transition:box-shadow 0.25s,transform 0.18s;'>`;
          html += `<div class='card-title' style='color:${acteur.color};font-size:1.08em;font-weight:800;letter-spacing:0.5px;display:flex;align-items:center;gap:8px;'><span style='display:inline-block;background:${acteur.color}22;border-radius:8px;padding:4px 8px;'><i class='fas ${acteur.icon}' style='color:${acteur.color};font-size:1em;'></i></span> ${acteur.label}</div>`;
          html += `<div class='card-value' style='color:${
            acteur.color
          };font-size:1.7em;font-weight:900;text-shadow:0 2px 12px ${
            acteur.color
          }22;margin-bottom:2px;'>${stats.total || 0}</div>`;
          html += `<div style='font-size:0.98em;font-weight:600;color:#374151;margin-bottom:6px;letter-spacing:0.2px;'>Opérations traitées</div>`;
          html += `<span class='card-arrow' style='top:12px;right:12px;'><i class='fas fa-chevron-right'></i></span>`;
          // Effet décoratif : vague SVG en bas de la carte
          html += `<svg viewBox="0 0 400 40" width="100%" height="22" style="position:absolute;bottom:0;left:0;z-index:0;opacity:0.13;"><path d="M0,30 Q100,0 200,30 T400,30 V40 H0Z" fill="${acteur.color}"/></svg>`;
          html += `</div>`;
        });
        html += `</div>`;
        // (plus de sélecteur de date ici)
        html += `</div>`;
        html += `<div id='detailsStatistiquesActeur' style='margin-top:32px;'></div>`;
        section.innerHTML = html;

        // Ajout de l'écouteur sur le sélecteur de date (toujours après le render)
        const dateSelector = document.getElementById(
          "date-filtre-statistiques"
        );
        if (dateSelector) {
          // Si la date n'est pas définie, on prend la date du jour
          if (!dateSelector.value) {
            dateSelector.value = dateStr || todayStr;
          }
          dateSelector.setAttribute("max", todayStr);
          // On retire tout ancien handler
          dateSelector.removeEventListener(
            "change",
            dateSelector._statsChangeHandler || (() => {})
          );
          dateSelector._statsChangeHandler = function () {
            ...
            // On ne limite pas à aujourd'hui, on laisse l'utilisateur choisir une date passée
            dateSelector.value = val;
            fetchStats(val);
          };
          dateSelector.addEventListener(
        ...
        // Interactivité
        const cards = section.querySelectorAll(".stats-acteur-card");
        const detailsDiv = document.getElementById("detailsStatistiquesActeur");
        cards.forEach((card) => {
          card.addEventListener("click", function () {
            cards.forEach((c) => c.classList.remove("active"));
            card.classList.add("active");
            const acteurKey = card.getAttribute("data-acteur");
            afficherDetailsStatistiquesActeur(
              acteurKey,
              data,
              detailsDiv,
              acteurs
      ...
      .catch(() => {
        section.innerHTML = `<div style='color:#dc3545;text-align:center;'>Impossible de charger les statistiques.</div>`;
      });
  }
  // Initialisation : charger pour la date sélectionnée si déjà présente, sinon aujourd'hui
  let dateSelector = document.getElementById("date-filtre-statistiques");
  let initialDate = todayStr;
  if (dateSelector && dateSelector.value) {
    initialDate = dateSelector.value;
  }
  fetchStats(initialDate);
}

// Affichage détaillé des statistiques pour un acteur
function afficherDetailsStatistiquesActeur(
  acteurKey,
  data,
  detailsDiv,
  acteurs
  ...
  const acteur = acteurs.find((a) => a.key === acteurKey);
  const stats = data[acteurKey] || {};
  // LOG DEBUG: Affichage brut des stats.details pour ce rôle
  console.log("--- DEBUG: acteurKey =", acteurKey);
  console.log("--- DEBUG: stats.details =", stats.details);
  if (Array.isArray(stats.details)) {
    stats.details.forEach((ligne, idx) => {
      console.log(`--- Ligne ${idx} ---`);
      for (const k in ligne) {
        if (Object.prototype.hasOwnProperty.call(ligne, k)) {
          console.log(`  ${k}:`, ligne[k]);
        }
      }
    });
  }
  let html = `<div style='background:${acteur.bg};border-radius:18px;box-shadow:0 6px 32px ${acteur.color}22;padding:38px 28px 28px 28px;max-width:1100px;margin:0 auto;position:relative;overflow:hidden;'>`;
  html += `<div style='font-size:1.5em;font-weight:800;color:${acteur.color};margin-bottom:18px;display:flex;align-items:center;gap:12px;'><span style='display:inline-block;background:${acteur.color}22;border-radius:8px;padding:8px 12px;'><i class='fas ${acteur.icon}'></i></span> ${acteur.label} — Détail des opérations</div>`;
  // Colonnes principales (affichage tableau) et secondaires (affichage détail)
  let colonnesPrincipales = [];
  let colonnesSecondaires = [];
  if (acteurKey === "agentAcconier") {
    // Colonnes principales du tableau de suivi pour l'acconier (ordre logique et métier)
    colonnesPrincipales = [
      "Agent",
      "Client (Nom)",
      "Client (Tél)",
    ...
    // Remplir les champs Immatriculation, Tél. Chauffeur, Heure Livraison à partir des valeurs du tableau de suivi si possible
    // On suppose que les données sources sont dans les propriétés :
    //   Immatriculation => truck_registration
    //   Tél. Chauffeur => driver_phone
    //   Heure Livraison => delivery_time
    if (Array.isArray(stats.details)) {
      stats.details = stats.details.map((ligne, idx) => {
        // Log temporaire pour diagnostic : affiche toutes les clés et valeurs de chaque ligne
        if (idx === 0) {
          console.log("--- Diagnostic Agent en Douanes (ligne 0) ---");
          console.log("Clés de la ligne :", Object.keys(ligne));
          console.log("Valeurs de la ligne :", ligne);
        }
        // Mapping robuste pour Agent en Douanes (toutes variantes)
        let agentDouanes =
          ligne["Agent en Douanes"] ||
          ligne["agent_en_douanes"] ||
          ligne["agent en douanes"] ||
          ligne["AgentEnDouanes"] ||
          ligne["agentEnDouanes"] ||
          ligne["agentdouanes"] ||
          ligne["agent_douanes"] ||
          ligne["Agent Douanes"] ||
          ligne["agent douanes"] ||
          "-";
        // Mapping robuste pour Immatriculation
        let immatriculation =
          ligne["Immatriculation"] ||
          ligne["truck_registration"] ||
          ligne["immatriculation"] ||
          ligne["truckregistration"] ||
          ligne["immatriculationcamion"] ||
          "-";
        // Mapping robuste pour Tél. Chauffeur
        let telChauffeur =
          ligne["Tél. Chauffeur"] ||
          ligne["driver_phone"] ||
          ligne["tél. chauffeur"] ||
          ligne["tel_chauffeur"] ||
          ligne["telchauffeur"] ||
          ligne["telephone_chauffeur"] ||
          "-";
        // Mapping robuste pour Heure Livraison
        let heureLivraison =
          ligne["Heure Livraison"] ||
          ligne["delivery_time"] ||
          ligne["heure livraison"] ||
          ligne["heurelivraison"] ||
          "-";
        return {
          ...ligne,
          "Agent en Douanes": agentDouanes,
          Immatriculation: immatriculation,
          "Tél. Chauffeur": telChauffeur,
          "Heure Livraison": heureLivraison,
        };
      });
    }
  }
  const lignes = Array.isArray(stats.details) ? stats.details : [];
  // Suppression de la barre de recherche et du tri : on affiche directement le tableau sans ces éléments
  html += `<div class="table-scroll-vertical"><table id='tableDetailsStatistiques' style='width:100%;border-collapse:separate;border-spacing:0;font-size:1.13em;margin-bottom:22px;background:rgba(255,255,255,0.97);box-shadow:0 2px 10px #0002;'>`;
  // Ajout du style CSS pour la scrollbar, le scroll vertical et l'embellissement du tableau si pas déjà présent
  if (!document.getElementById("scrollbar-style-statistiques")) {
    const style = document.createElement("style");
    style.id = "scrollbar-style-statistiques";
    style.innerHTML = `
      ...
      .table-scroll-vertical th, .table-scroll-vertical td {
      ...
      #tableDetailsStatistiques tr:nth-child(even) td {
        background: #f7faff;
      }
      #tableDetailsStatistiques tr:hover td {
        background: #eaf1fb;
        color: #2563eb;
      }
      #tableDetailsStatistiques td strong {
        font-weight: 800;
      }
      #tableDetailsStatistiques td .badge {
        display: inline-block;
        background: #eaf1fb;
        color: #2563eb;
        border-radius: 7px;
      ...
      #tableDetailsStatistiques th:last-child, #tableDetailsStatistiques td:last-child {
        border-right: none;
      }
  ...
  // Supprimer la colonne Détails pour Responsable Acconier
  if (acteurKey !== "responsableAcconier") {
    html += `<th>Détails</th>`;
  }
  html += `</tr></thead><tbody id='tbodyDetailsStatistiques'>`;
  // Génération des lignes (fonction pour réutilisation lors du tri/recherche)
  function renderRows(rows) {
    if (rows.length === 0) {
      // Adapter le colspan selon la présence ou non de la colonne Détails
      const colspan =
        acteurKey === "responsableAcconier"
          ? colonnesPrincipales.length
          : colonnesPrincipales.length + 1;
      return `<tr><td colspan='${colspan}' style='text-align:center;color:#a0aec0;padding:18px 0;'>Aucune donnée à afficher.</td></tr>`;
    }
        ...
        colonnesPrincipales.forEach((col) => {
          ...
          if (
            ...
            if (
              ...
              ligne["observation_acconier"] !== null
            ) {
              val = ligne["observation_acconier"];
            } else if (ligne[col] !== undefined && ligne[col] !== null) {
              val = ligne[col];
            } else {
              val = "-";
          ...
          // --- Affichage synchronisé des statuts de conteneur pour Responsable Livraison ---
          else if (
            acteurKey === "responsableLivraison" &&
            col === "Statut (Livraison)"
          ) {
            ...
            function getStatusInfo(status) {
              ...
              let info =
                statusMap[normalizedStatus] || statusMap[trimmedStatus];
              if (!info) {
                if (
                  normalizedStatus.includes("progress") ||
                  normalizedStatus.includes("cours")
                ) {
                  info = statusMap["inprogress"];
                } else {
                  info = {
                    text:
            ...
            let delivered = 0;
            let tcList = [];
            if (Array.isArray(ligne["Numéro TC(s)"])) {
              tcList = ligne["Numéro TC(s)"].filter(Boolean);
            } else if (typeof ligne["Numéro TC(s)"] === "string") {
              tcList = ligne["Numéro TC(s)"].split(/[,;\s]+/).filter(Boolean);
            }
            if (
              tcList.length === 0 &&
              (ligne["Nombre de conteneurs"] || ligne["nombre de conteneurs"])
            ) {
              total = parseInt(
                ligne["Nombre de conteneurs"] || ligne["nombre de conteneurs"]
              );
            ...
            let statutsConteneurs = [];
            if (
              ligne.container_statuses &&
              typeof ligne.container_statuses === "object"
            ) {
              tcList.forEach((tc) => {
                let statut = ligne.container_statuses[tc] || "-";
                const info = getStatusInfo(statut);
                statutsConteneurs.push({ tc, ...info });
                if (info.badgeClass === "delivered") delivered++;
              });
            } else if (Array.isArray(ligne.container_statuses)) {
              ligne.container_statuses.forEach((statut, idx) => {
                const tc = tcList[idx] || `TC${idx + 1}`;
                const info = getStatusInfo(statut);
                statutsConteneurs.push({ tc, ...info });
                if (info.badgeClass === "delivered") delivered++;
              });
            ...
            if (total === 0) {
              ...
              if (delivered === total && total > 0) {
                resume = `<span style='color:#22c55e;font-weight:700;'><i class='fas fa-check-circle' style='color:#22c55e;margin-right:4px;'></i>Livré</span>`;
              } else {
                resume = `<span style='color:#2563eb;font-weight:700;'>${delivered} sur ${total} livrés</span>`;
              }
              let detail = statutsConteneurs
                .map(
                  (s) =>
                    `<span class=\"badge ${s.badgeClass}\" style=\"margin-right:4px;display:inline-flex;align-items:center;background:#eaf1fb;padding:2px 8px;border-radius:7px;font-weight:600;color:${s.hexColor};\"><i class=\"fas ${s.iconClass}\" style=\"margin-right:5px;color:${s.hexColor};\"></i>${s.tc}: ${s.text}</span>`
                )
                .join("");
              val = `<div style='display:flex;flex-direction:column;gap:2px;'>${resume}<div>${detail}</div></div>`;
            }
          }
          if (val === undefined) {
            const normalize = (s) =>
            ...
            for (const k in ligne) {
              if (normalize(k) === colNorm) {
                val = ligne[k];
                break;
              }
            }
          }
          if (col.toLowerCase().includes("date") && val && val !== "-") {
            try {
              val = new Date(val).toLocaleDateString("fr-FR");
            } catch (e) {}
          }
          if (col.toLowerCase().includes("heure") && val && val !== "-") {
            try {
              val = new Date("1970-01-01T" + val).toLocaleTimeString("fr-FR", {
                hour: "2-digit",
                minute: "2-digit",
              });
            } catch (e) {}
          }
          tds += `<td style='max-width:220px;overflow-x:auto;white-space:pre-line;word-break:break-word;'>${
            val !== undefined && val !== null && val !== "" ? val : "-"
          }</td>`;
        });
        // Cellule détails (bouton) : uniquement si ce n'est PAS Responsable Acconier
        if (acteurKey !== "responsableAcconier") {
          tds += `<td style='text-align:center;'><button class='btn-details-stats' data-row='${idx}' style='padding:4px 10px;font-size:0.98em;border-radius:7px;border:1px solid #d1d5db;background:#f7faff;color:${acteur.color};cursor:pointer;transition:background 0.18s;'>Détails</button></td>`;
        }
        return `<tr>${tds}</tr>`;
      })
      .join("");
  }
  html += renderRows(lignes);
  html += `</tbody></table></div></div></div>`;
  // Ajout d'une vague décorative en bas
  html += `<svg viewBox="0 0 400 40" width="100%" height="32" style="position:absolute;bottom:0;left:0;z-index:0;opacity:0.10;"><path d="M0,30 Q100,0 200,30 T400,30 V40 H0Z" fill="${acteur.color}"/></svg>`;
  html += `</div>`;
  detailsDiv.innerHTML = html;
  // --- Détail sur clic bouton ---
  setTimeout(() => {
    const btns = detailsDiv.querySelectorAll(".btn-details-stats");
    if (btns && btns.length) {
      btns.forEach((btn) => {
        btn.onclick = function () {
          const idx = parseInt(this.getAttribute("data-row"), 10);
          const ligne = lignes[idx];
          let detailHtml = `<div style='font-size:1.25em;font-weight:800;color:${acteur.color};margin-bottom:18px;text-align:center;letter-spacing:0.5px;'><i class='fas fa-info-circle' style='margin-right:8px;color:${acteur.color};'></i>Détails de l'opération</div>`;
          detailHtml += `<ul style='list-style:none;padding:0;margin:0;max-width:420px;margin:0 auto;'>`;
          colonnesPrincipales.forEach((col) => {
            let val = ligne[col];
            if (
              acteurKey === "responsableAcconier" &&
              ...
              if (
                Array.isArray(ligne["Numéro TC(s)"]) &&
                ligne.container_statuses &&
                typeof ligne.container_statuses === "object"
              ) {
                let tcList = ligne["Numéro TC(s)"].filter(Boolean);
                let rawStatus = tcList
                  .map(
                    (tc) =>
                      `${tc}: ${tradFR(ligne.container_statuses[tc] || "-")}`
                  )
                  .join(", ");
                val = rawStatus || "-";
              } else if (
                typeof ligne["Numéro TC(s)"] === "string" &&
                ligne.container_statuses &&
                typeof ligne.container_statuses === "object"
              ) {
                let tcList = ligne["Numéro TC(s)"]
                  .split(/[,;\s]+/)
                  .filter(Boolean);
                let rawStatus = tcList
                  .map(
                ...
                val = ligne.container_statuses.map(tradFR).join(", ");
              } else if (typeof ligne["Statut (Livraison)"] === "string") {
                val = tradFR(ligne["Statut (Livraison)"]);
              } else {
                val = "-";
              }
            } else {
            ...
            if (val === undefined || val === null || val === "") val = "-";
            detailHtml += `<li style='display:flex;align-items:flex-start;gap:10px;margin-bottom:10px;background:#f7faff;border-radius:10px;padding:10px 14px;box-shadow:0 2px 8px #2563eb11;'>
              <span style='min-width:170px;display:inline-block;font-weight:700;color:${acteur.color};'>${col}</span>
              <span style='flex:1;font-weight:600;color:#374151;'>${val}</span>
            </li>`;
          });
          detailHtml += `</ul>`;
          // Popup simple
          ...
          if (!popup) {
            ...
            popup.style.justifyContent = "center";
            popup.innerHTML = `
              <div style='background:linear-gradient(135deg,#f7faff 60%,#e3e9f7 100%);border-radius:22px;box-shadow:0 12px 48px ${acteur.color}55, 0 2px 8px #0002;padding:38px 32px 28px 32px;max-width:540px;width:96vw;max-height:92vh;overflow-y:auto;position:relative;display:flex;flex-direction:column;align-items:center;'>
                <button id='closePopupDetailsStats' style='position:absolute;top:18px;right:18px;background:none;border:none;font-size:2em;color:${acteur.color};cursor:pointer;transition:color 0.2s;z-index:2;' onmouseover="this.style.color='#dc3545'" onmouseout="this.style.color='${acteur.color}'">&times;</button>
                <div style='width:100%;display:flex;align-items:center;gap:12px;margin-bottom:18px;'>
                  <span style='display:inline-block;background:${acteur.color}22;border-radius:8px;padding:8px 12px;'><i class='fas ${acteur.icon}' style='color:${acteur.color};font-size:1.3em;'></i></span>
                  <span style='font-size:1.25em;font-weight:800;color:${acteur.color};letter-spacing:0.5px;'>Détails de l'opération</span>
                </div>
                <div id='contenuPopupDetailsStats' style='width:100%;margin-bottom:8px;'></div>
                <svg viewBox="0 0 400 40" width="100%" height="32" style="position:absolute;bottom:0;left:0;z-index:0;opacity:0.13;"><path d="M0,30 Q100,0 200,30 T400,30 V40 H0Z" fill="${acteur.color}"/></svg>
              </div>
            `;
            document.body.appendChild(popup);
          }
          document.getElementById(
            "contenuPopupDetailsStats"
          ).innerHTML = `<div style='display:flex;flex-direction:column;gap:8px;'>${detailHtml}</div>`;
          popup.style.display = "flex";
          const closeBtn = document.getElementById("closePopupDetailsStats");
          if (closeBtn) {
            closeBtn.onclick = () => {
              popup.style.display = "none";
            };
  ...
  // Suppression des filtres/recherche/tri : on affiche simplement toutes les lignes sans barre de recherche ni tri
  const tbody = document.getElementById("tbodyDetailsStatistiques");
    ...
    if (btns && btns.length) {
      btns.forEach((btn) => {
        btn.addEventListener("click", function () {
          const idx = parseInt(this.getAttribute("data-row"), 10);
          const ligne = lignes[idx];
          let detailHtml = `<div style='font-size:1.15em;font-weight:700;color:${acteur.color};margin-bottom:10px;text-align:center;'>Détails de l'opération</div>`;
          colonnesPrincipales.forEach((col) => {
            let val = ligne[col];
            if (val === undefined) {
              const normalize = (s) =>
            ...
            detailHtml += `<div style='margin-bottom:6px;'><strong>${col} :</strong> <span>${
              val !== undefined && val !== null && val !== "" ? val : "-"
            }</span></div>`;
          });
          // Popup simple
          ...
          if (!popup) {
            ...
            popup.style.alignItems = "center";
            popup.style.justifyContent = "center";
            popup.innerHTML = `<div style='background:#fff;border-radius:18px;box-shadow:0 8px 40px ${acteur.color}44;padding:38px 32px 28px 32px;max-width:500px;width:96vw;max-height:92vh;overflow-y:auto;position:relative;'><button id='closePopupDetailsStats' style='position:absolute;top:18px;right:18px;background:none;border:none;font-size:1.5em;color:${acteur.color};cursor:pointer;'>&times;</button><div id='contenuPopupDetailsStats'></div></div>`;
            document.body.appendChild(popup);
          }
          document.getElementById("contenuPopupDetailsStats").innerHTML =
            detailHtml;
          popup.style.display = "flex";
          const closeBtn = document.getElementById("closePopupDetailsStats");
          if (closeBtn) {
  ...
  // Affichage direct de toutes les lignes
  tbody.innerHTML = renderRows(lignes);
  attachDetailButtons();
  // Ajout d'une vague décorative en bas
  html += `<svg viewBox="0 0 400 40" width="100%" height="32" style="position:absolute;bottom:0;left:0;z-index:0;opacity:0.10;"><path d="M0,30 Q100,0 200,30 T400,30 V40 H0Z" fill="${acteur.color}"/></svg>`;
  html += `</div>`;
  detailsDiv.innerHTML = html;
} // Affichage dynamique de la section statistiques
document.addEventListener("DOMContentLoaded", () => {
  const btnStats = document.getElementById("lienStatistiques");
  const sectionStats = document.getElementById("statistiquesSection");
  if (btnStats && sectionStats) {
    btnStats.addEventListener("click", (e) => {
      e.preventDefault();
      masquerAccueil();
      masquerToutesSectionsDynamiques();
      sectionStats.style.display = "block";
      // Mettre à jour dynamiquement la date du filtre à chaque ouverture
      setTimeout(() => {
        let dateSelector = document.getElementById("date-filtre-statistiques");
        const today = new Date();
        const year = today.getFullYear();
        ...
        if (dateSelector) {
          dateSelector.setAttribute("max", todayStr);
          // Ne pas forcer la valeur à aujourd'hui si déjà une date sélectionnée
          // Juste s'assurer que le max est à jour
          // Ajout : navigation clavier (flèches) pour changer la date
          if (!dateSelector._arrowKeyHandler) {
            dateSelector._arrowKeyHandler = function (e) {
              if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
                let current = dateSelector.value;
                if (!current) return;
                let d = new Date(current);
                if (isNaN(d)) return;
                if (e.key === "ArrowLeft") d.setDate(d.getDate() - 1);
                if (e.key === "ArrowRight") d.setDate(d.getDate() + 1);
                // Ne pas dépasser la date max
                const max = dateSelector.getAttribute("max");
                const min = dateSelector.getAttribute("min");
                let dStr = d.toISOString().slice(0, 10);
                if (max && dStr > max) dStr = max;
                if (min && dStr < min) dStr = min;
                dateSelector.value = dStr;
                if (typeof dateSelector._statsChangeHandler === "function") {
                  dateSelector._statsChangeHandler();
                }
                e.preventDefault();
              }
            ...
            dateSelector.addEventListener(
              "keydown",
              dateSelector._arrowKeyHandler
            );
          }
          // Déclencher le changement pour recharger les stats si aucune date sélectionnée
          if (!dateSelector.value) {
            dateSelector.value = todayStr;
            if (typeof dateSelector._statsChangeHandler === "function") {
              dateSelector._statsChangeHandler();
            } else {
              chargerStatistiquesActeurs();
            }
          }
        } else {
          chargerStatistiquesActeurs();
