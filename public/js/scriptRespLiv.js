...
// --- AJOUT : ENVOI FORMULAIRE ORDRE DE LIVRAISON EN TEMPS RÉEL ---
// Ce bloc s'exécute uniquement si le formulaire d'ordre de livraison est présent sur la page
document.addEventListener("DOMContentLoaded", function () {
  const form = document.querySelector("#ordreLivraisonForm");
  if (form) {
    // Empêche le rechargement et envoie les données en AJAX
    form.addEventListener("submit", async function (e) {
      ...
      // Envoi AJAX (POST) vers le backend (à adapter côté serveur)
      try {
        const response = await fetch("/ordre-livraison", {
          method: "POST",
        ...
        if (res.success) {
          // Envoi aussi via WebSocket si disponible (pour notification temps réel)
          if (window.ws && window.ws.readyState === 1) {
            window.ws.send(
              JSON.stringify({
                type: "ordre_livraison_created",
                livraison: res.livraison || data,
              })
            );
          }
          // Message de confirmation instantané
          showAccessMessage(
            "✅ Ordre de livraison transmis avec succès !",
            "green"
          );
          ...
          showAccessMessage(
            res.message || "Erreur lors de l'envoi de l'ordre de livraison.",
            "red"
          );
        }
      } catch (err) {
        showAccessMessage(
          "Erreur réseau lors de l'envoi de l'ordre de livraison.",
          "red"
        );
      }
    });
...
// --- RÉCEPTION TEMPS RÉEL SUR resp_liv.html ---
// Ajout d'un écouteur WebSocket pour afficher la nouvelle livraison et le message temps réel
if (typeof window !== "undefined") {
  // On attend que la connexion WebSocket soit prête (déjà gérée plus haut)
  function handleOrdreLivraisonRealtime(data) {
    if (data && data.type === "ordre_livraison_created" && data.livraison) {
      // Ajoute la livraison reçue à la liste globale et rafraîchit le tableau
      if (window.allDeliveries && Array.isArray(window.allDeliveries)) {
        window.allDeliveries.push(data.livraison);
        // Rafraîchir le tableau (si filtres date présents)
        const dateStartInput = document.getElementById(
          "mainTableDateStartFilter"
        );
        const dateEndInput = document.getElementById("mainTableDateEndFilter");
      ...
      // Affiche le message temps réel
      showAccessMessage("Un acconier a établi un ordre de livraison.", "green");
    }
  }
  // Patch du WebSocket pour écouter ce type de message
  if (window.ws) {
    const oldOnMessage = window.ws.onmessage;
    window.ws.onmessage = function (event) {
      let handled = false;
      try {
        const data = JSON.parse(event.data);
        if (data.type === "ordre_livraison_created") {
          handleOrdreLivraisonRealtime(data);
          handled = true;
        }
      } catch (e) {}
      if (!handled && typeof oldOnMessage === "function") {
        oldOnMessage.call(window.ws, event);
      }
    };
  } else {
    // Si ws pas encore prêt, on attend qu'il soit créé
