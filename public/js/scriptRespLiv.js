  ...
  deliveries.forEach((delivery, i) => {
    ...
    AGENT_TABLE_COLUMNS.forEach((col, idx) => {
      ...
      if (col.id === "row_number") {
        ...
        td.addEventListener("click", function onEditCell(e) {
          e.stopPropagation();
          if (td.querySelector("input,textarea,select")) return; // déjà en édition
          let input;
          if (col.id === "statut" || col.id === "acconier_status") {
            // Pour statut, on peut proposer un select (à adapter selon tes valeurs)
            input = document.createElement("select");
            ["pending", "mise_en_livraison", "livré", "annulé"].forEach(
              (opt) => {
                const option = document.createElement("option");
                option.value = opt;
                option.textContent = opt.charAt(0).toUpperCase() + opt.slice(1);
                if (delivery[col.id] === opt) option.selected = true;
                input.appendChild(option);
              }
            );
          } else if (col.id === "observation") {
            input = document.createElement("textarea");
            input.value = delivery[col.id] || "";
            input.rows = 2;
            input.style.width = "98%";
          } else if (col.id === "delivery_date") {
            input = document.createElement("input");
            input.type = "date";
            // Conversion date pour input type date
            let d = delivery[col.id];
            if (d) {
          ...
          input.style.border = "1.2px solid #2563eb";
          ...
          input.addEventListener("keydown", function (ev) {
            if (ev.key === "Enter" && col.id !== "observation") {
              ev.preventDefault();
              input.blur();
            }
          });
          input.addEventListener("blur", function () {
            ...
            // MAJ locale (à adapter si tu veux envoyer au serveur)
            delivery[col.id] = newValue;
