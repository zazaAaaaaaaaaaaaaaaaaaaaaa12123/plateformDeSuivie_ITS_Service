...
// Gestion sélection et suppression
if (!window.selectedRowsForDelete) {
  window.selectedRowsForDelete = new Set();
}
// Ajout du bouton de suppression si pas déjà présent
...
if (!deleteBtn) {
  ...
  deleteBtn.style.background = "linear-gradient(90deg,#eab308 0%,#2563eb 100%)";
  deleteBtn.style.color = "#fff";
  deleteBtn.style.fontWeight = "bold";
  deleteBtn.style.fontSize = "1.08em";
  deleteBtn.style.border = "none";
  deleteBtn.style.borderRadius = "12px";
  deleteBtn.style.padding = "0.7em 2em";
  deleteBtn.style.boxShadow = "0 2px 12px rgba(37,99,235,0.13)";
  deleteBtn.style.display = "none";
  deleteBtn.onclick = async function () {
    // Suppression des lignes sélectionnées
    const idsToDelete = Array.from(window.selectedRowsForDelete);
    for (const id of idsToDelete) {
      try {
        await fetch(`/deliveries/${id}`, { method: "DELETE" });
      } catch (e) {
        console.error("Erreur suppression", e);
      }
    }
    window.selectedRowsForDelete.clear();
    // Rechargement des données
    if (typeof loadAllDeliveries === "function") {
      await loadAllDeliveries();
      if (typeof updateTableForDateRange === "function") {
        // On conserve la plage de dates actuelle
...
deliveries.forEach((delivery, i) => {
  ...
  AGENT_TABLE_COLUMNS.forEach((col, idx) => {
    ...
    if (col.id === "select_row") {
      // Ajout de la checkbox de sélection
      const checkbox = document.createElement("input");
      checkbox.type = "checkbox";
      checkbox.style.transform = "scale(1.3)";
      checkbox.style.margin = "0 auto";
      checkbox.checked = window.selectedRowsForDelete.has(delivery.id);
      checkbox.onclick = function (e) {
        if (checkbox.checked) {
          window.selectedRowsForDelete.add(delivery.id);
        } else {
          window.selectedRowsForDelete.delete(delivery.id);
        }
        // Affichage du bouton suppression si au moins une sélection
        deleteBtn.style.display =
          window.selectedRowsForDelete.size > 0 ? "block" : "none";
      };
      td.appendChild(checkbox);
      td.classList.add("select-row-col");
      ...
      if (blList.length > 1) {
        ...
        btn.innerHTML =
          blList
            ...
            .join("") +
          (blList.length > 2
            ? ` <span class=\"tc-tag tc-tag-more\">+${blList.length - 2}</span>`
            : "") +
          ' <i class="fas fa-chevron-down tc-chevron"></i>';
        const popup = document.createElement("div");
        popup.className = "tc-popup";
      ...
      function showBLDetailPopup(delivery, blNumber) {
        ...
        saveBtn.onclick = async () => {
          let statutToSend = select.value === "aucun" ? "aucun" : select.value;
    ...
    function showContainerDetailPopup(delivery, containerNumber) {
      ...
      header.style.padding =
        window.innerWidth <= 600 ? "12px 12px 8px 12px" : "18px 28px 12px 28px";
      header.style.fontWeight = "bold";
      header.style.fontSize = window.innerWidth <= 600 ? "1.01rem" : "1.15rem";
      header.style.display = "flex";
      header.style.flexDirection = "column";
      ...
      closeBtn.style.border = "none";
      closeBtn.style.color = "#fff";
      closeBtn.style.fontSize = window.innerWidth <= 600 ? "1.5rem" : "2.1rem";
      closeBtn.style.cursor = "pointer";
      closeBtn.style.position = "absolute";
