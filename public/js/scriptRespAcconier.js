...
function supprimerDossiersSelectionnes() {
  ...
  const containerNames = dossiersASupprimer
    .map(
      (d) => d.container_number || d.ref_conteneur || d.dossier_number || "N/A"
    )
    .join(", ");
  if (
    !confirm(
  ...
  const nouveauxDossiers = dossiers.filter((dossier) => {
    ...
    return !dossiersASupprimer.some((dossierASupprimer) => {
      const aSupprImerId =
        (dossierASupprimer.container_number ||
          dossierASupprimer.ref_conteneur ||
          dossierASupprimer.dossier_number) +
        (dossierASupprimer.date_mise_en_liv ||
          dossierASupprimer.dateAjoutMiseEnLiv ||
          "");
  ...
  console.log(
    `🗑️ [SUPPRESSION MANUELLE] ${dossiersASupprimer.length} dossier(s) supprimé(s) avec succès`
  );
  ...
  dossiers.forEach((dossier) => {
    ...
    if (dossier.dateExpirationMiseEnLiv) {
      ...
      if (maintenant >= dateExpiration) {
        ...
        dossiersSupprimes++;
        console.log(
          `🗑️ [NETTOYAGE AUTO] Dossier expiré supprimé: ${
            dossier.container_number || dossier.dossier_number
          }`
        );
      } else {
...
function calculerTempsRestant(dateExpiration) {
  ...
  if (jours > 0) {
    ...
    details = `Suppression dans moins de ${heures + 1}h`;
  } else {
    texte = `⚡ ${minutes}m restantes`;
    couleur = "danger";
    details = "Suppression imminente !";
  }
        ...
        <div class="d-flex justify-content-between align-items-start">
          <div class="me-3">
            ...
            <div class="d-flex flex-column">
              ...
              ${
                // ⏰ AFFICHAGE AMÉLIORÉ DU TEMPS RESTANT AVANT SUPPRESSION AUTOMATIQUE
                (() => {
                  const tempsRestant = calculerTempsRestant(
                    dossier.dateExpirationMiseEnLiv
                  );
                  if (tempsRestant) {
                    return `
